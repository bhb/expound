...
"
               (s/explain-str
                ...
                "sally@"))))
      ;; s/int-in expands to two predicates joined by s/and
      (expound/def :predicate-messages/score (s/int-in 0 100) "should be between 0 and 99")
      (is (= "-- Spec failed --------------------
...
"
             (s/explain-str
              ...
              100))))
    (testing "replacing original message w/ function"
      (expound/def :predicate-messages/small-num
        (spt/spec (s/int-in 0 10) {:reason "between 0 and 9"})
        (fn [old-msg problem]
          (str "should be " (:reason problem))))
      (is (= "-- Spec failed --------------------
...
"
             (s/explain-str
              :predicate-messages/small-num
              101))))
    (testing "appending to original message"
      (expound/def :predicate-messages/kw
        keyword?
        (fn [old-msg problem]
          (str old-msg " (because keywords are easier to read)")))
      (is (= "-- Spec failed --------------------
...
should satisfy
  ...
  keyword? (because keywords are easier to read)

-------------------------
Detected 1 error
"
             (s/explain-str
              :predicate-messages/kw
              101))))))
