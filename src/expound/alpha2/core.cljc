...
(s/def :expound.printer/show-specs? boolean?)
...
(defn ^:private pr-pred [pred spec]
  (if (= :clojure.spec.alpha/unknown pred)
    (pr-pred* spec)
    (pr-pred* pred)))
...
(defn ^:private show-spec-name [spec-name value]
  (if spec-name
    (str
     (case spec-name
       :clojure.spec.alpha/pred "" ; Used in s/assert
...
(defn ^:private spec-w-error-message? [via pred]
  (boolean (let [last-spec (last via)]
             (and (not= :clojure.spec.alpha/unknown pred)
...
(defn ^:private no-method [_spec-name _form _path problem]
  (let [dispatch-val (last (:expound/path problem))
        sp (s/get-spec (last (:expound/via problem)))
    ...
    ;; https://clojuredocs.org/clojure.alpha.spec/multi-spec#example-5b750e5be4b00ac801ed9e60
    ;;
...
(defn ^:private spec-name [ed]
  (if (#{:instrument} (:clojure.spec.alpha/failure ed))
    (cond
      ;; This works for clojure.spec <= 0.2.176
      ;; and CLJS <= 1.10.439
      (:clojure.spec.alpha/args ed)
      :args

      :else
      ;; for earlier versions
      (-> ed :clojure.spec.alpha/problems first :path first))
  ...
  (if-not explain-data
    ...
    (let [explain-data' (problems/annotate explain-data)
          {:expound/keys [caller form]
           :clojure.spec.alpha/keys [failure]} explain-data'
...
(defn ^:private print-check-result [check-result]
  (let [{:keys [sym spec failure] :or {sym '<unknown>}} check-result
        ...
        failure-reason (:clojure.spec.alpha/failure explain-data)
        sym (or sym '<unknown>)]
    (str
     ...
     (cond
       ...
       (str
        #?(:clj
           (let [path (:clojure.spec.alpha/path explain-data)]
             (str
       ...
       (and explain-data
            (= :check-failed (-> explain-data :clojure.spec.alpha/failure)))
       (with-out-str
...
(defn custom-printer
  ...
  Options:
   ...
   - `:show-specs?`       - if `true`, display \"Relevant specs\" section. Otherwise, omit that section.
